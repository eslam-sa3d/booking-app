...
- Admin dashboard manually verified against the real emulator: sign in as the seeded admin → live dashboard stats → Classes list renders seeded data → create a class (incl. Arabic title) → it appears live → delete it → it's gone. `admin-dashboard/integration_test/admin_golden_path_test.dart` automates the same flow via `flutter drive` (requires a working `chromedriver` on PATH — see `admin-dashboard/README.md`).
- This verification pass caught and fixed two real bugs: a placeholder Firebase API key format that crashed iOS at startup, and a race in `backend/functions/scripts/seed.js` where `onUserCreate`'s async claim-write could overwrite the seeded admin's `role: 'admin'` claim back to `'customer'` (seed.js now waits for the trigger's Firestore write to land before setting the real claim, so it always applies last).
